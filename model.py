...
def cnn_model(input_shape, learning_rate = 0.00001, dropout = 0.5, num_classes = 4):
    ...
    cnn_model.add(Conv2D(16, kernel_size = (2 ,2), input_shape = input_shape, padding="same"))
    cnn_model.add(MaxPooling2D(pool_size=(2, 2),padding="same"))
    cnn_model.add(Conv2D(32, kernel_size = (2, 2), input_shape = input_shape, padding="same"))
    cnn_model.add(MaxPooling2D(pool_size=(2, 2),padding="same"))
    cnn_model.add(Conv2D(32, kernel_size = (2, 2), input_shape = input_shape, padding="same"))
    cnn_model.add(MaxPooling2D(pool_size=(2, 2), padding="same"))
...
def model_fit(cnn_model, traindata, trainlabel, model_file_name, mode,epoch = 200, batch_size = 50):
    print("training..")
